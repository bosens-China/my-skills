...
不同模板可能引用不同变量。项目没有模板元数据服务时，统一发送以下三个字段，避免切换模板后缺少参数：
...
默认发送包含 `to`、`name`、`code` 的 JSON 数据。若项目维护了可靠的模板变量元数据，则按元数据发送该模板需要的字段。多个手机号使用英文逗号分隔，单次最多 5 个。

接口也支持 GET：

```text
GET https://push.spug.cc/sms/{template_code}?to=...&name=...&code=...
```
