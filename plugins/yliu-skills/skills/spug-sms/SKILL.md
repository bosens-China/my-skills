...
4. 默认将发送函数设计为接收并始终发送 `to`、`name`、`code`、`number`。只有项目维护了“模板 → 所需变量”配置时，才根据模板元数据选择字段。
...
- 在发送前校验：`to` 中每个手机号均为 11 位数字，`name` 为 1–10 位中文、英文字母或数字，`code` 为 4–6 位英文字母或数字，`number` 为数字或中文数字表示的有效时长。
